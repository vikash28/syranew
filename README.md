# Syra Spring MVC app | Syra Web App #

The Syra Web application - BigData ETL Migration tool.
...
To build the backend and start the server, run the following command on the root folder of the repository:
    ...
    mvn clean install tomcat7:run-war -Dspring.profiles.active=production
...
To see a user with existing data (4 connections, 4 mappings, few schedules), login with the following credentials:

    username: Nick / password: Welcome2

### Frontend Overview ###

The Syra Web app is a web application with an AngularJs-based frontend and a Spring/Hibernate based backend. The application is responsive, as it adapts to different screen sizes.
...
The REST API of the backend is composed of muliple services:
...
Url|Verb|Description
---|----|-----------
/user|GET|retrieves info for the currently logged-in user
/user|PUT|Used to save the user
...
Url|Verb|Description
---|----|-----------
/connection|GET|searches connections for the current user by date/time
/connection|POST|saves a modified set of connections, might included new ones
/connection|DELETE|deletes a set of connections

### Testing code coverage ###

Test classes written for all the Services exposed with a good code coverage
...
The application can be started in HTTPS only mode by using the flag httpsOnly=true. This works in both modes, this is an example of how to start the application in production mode and HTTPS only:

    mvn clean install tomcat7:run-war -Dspring.profiles.active=production -DhttpsOnly=true
