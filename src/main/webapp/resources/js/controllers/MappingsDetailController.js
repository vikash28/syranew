...
MetronicApp.controller('MappingsDetailController', ['$rootScope', '$scope', 'settings', '$http' , 'MappingService', 
						'ScheduleService', 'FolderService', 'UserService', '$timeout','$stateParams','$state', function ($rootScope, $scope, settings, 
							$http, MappingService, ScheduleService, FolderService, UserService, $timeout,$stateParams,$state) {
	$scope.mappingData = {};
    ...
    $scope.$on('$viewContentLoaded', function() {   
		...
		$http.get("/mappingsview").success(function(response){
			var mapD=response.mappingsViews;
			var d= $scope.mappingDetail = mapD.find(function(data) {
		          if (data.id == id) {
		            return true;
		          }
			 });
			  var mappingsArray=JSON.parse($scope.mappingDetail.mappings);
			  $scope.mappingsArray=JSON.parse($scope.mappingDetail.mappings)[0];
			  if(d.sourceConnectionType == "Hive" && d.targetConnectionType == "Redshift"){
				  
				  $scope.concatDataSrc={
						  'hiveTableDataLocation':concatValues(mappingsArray, "hiveTableDataLocation"),
						  'hiveTableName':concatValues(mappingsArray, "hiveTableName"),
						  'seperator':concatValues(mappingsArray, "seperator")
				  };
			  ...
			  if(d.targetConnectionType == "Redshift"){
				  ...
				  $scope.RedShifttargetTable=concatValues(mappingsArray, "targetTable", "Redshift");

			  }
			  else if(d.targetConnectionType == "Hive"){
			
				  $scope.HiveTarget={
						  'destDataLocation':concatValues(mappingsArray, "destDataLocation"),
						  'destTableName':concatValues(mappingsArray, "destTableName"),
						  'destSeperator':concatValues(mappingsArray, "destSeperator")
		...
		var mappings = MappingService.searchMappings().then(function(success) {
		        return success.mappings;
		      });
		    
		      mappings.then(function(response) {
		        var mapDetail = response.find(function(data) {
		          if (data.id == id) {
		            return true;
		          }
		        });
			           $scope.listSchedules(mapDetail);
		      });
		...
		/* $(document).mouseup(function (e)
{
		...
		$('#actionsSchedule').on( 'click', '.schedule', function () {
			var row = mappingTable.row("#mapping-"+id);
		...
		$('#actionsSchedule').on( 'click', '.run', function () {
			var row = mappingTable.row("#mapping-"+id);
		...
		//Edit mapping option
		$('#actionsSchedule').on( 'click', '.edit', function () {
			var row = mappingTable.row("#mapping-"+id);
			var mapping = row.data();
		});
	

		$('#actionsSchedule').on( 'click', '.delete', function () {
			var $tbody = $(this);
