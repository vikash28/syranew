...
// write a validation which will accept only numbers, backspace and delete in fileds values. But 0 shouldn't be allowed at 1st place.
		...
		$(document).on('keypress', '.only_digit', function(e){ 
		  ...
		  //  	} else if (e.which == 8 || e.which == 127 ) { // allow backspace and delete
				// return true;		   
...
// write a validation which will accept only numbers, backspace, delete and decimal in fileds values. But 0 shouldn't be allowed at 1st place.
		...
		$("#checkout_date" ).datepicker({dateFormat: 'dd-mm-yy', minDate: 0 }).bind("change", function (e) {
            var maxDate = $(this).val();
            $(".stay_date_to").val(maxDate);
	...
	//		alert(id+no+minDate+maxDate);
		...
		$(document).on('click', '.add_more_room', function(){ 
        	...
        	var room_id = $("#room_id_"+no_of_room).val();
        	var no_of_rooms = $("#no_of_rooms_"+no_of_room).val();
        	var net_rate = $("#net_rate_"+no_of_room).val();
        	var display_room_no = $("#display_room_no_"+no_of_room).text();
        	
        	
        	// before adding more room option we ned to make sure that stay date from and to is filled up.....
        	//alert(no_of_room+" "+stay_date_from+" "+stay_date_to+" "+room_id+" "+no_of_rooms+" "+net_rate+" "+display_room_no);
        	if($("#stay_date_from_"+no_of_room).val() != "" && $("#stay_date_to_"+no_of_room).val() != "" && $("#hotel_id").val() != "" && room_id != "" && no_of_rooms != "" && net_rate != "" && display_room_no != "" ){
				no_of_room++;	
...
//			$("#"+room_number).removeClass("btn-default");
	...
	function calculate_total_room_tarrif(no){
		...
		var checkout_date = $('#stay_date_to_'+no).val();
	 	...
	 	//alert("cid"+checkin_date+"cid"+checkout_date+"cid"+net_rate+"cid"+no_of_room);
		...
		$(".done").click(function(){
			...
			if($('.wizard li').hasClass("jump-here")){
				...
				if (id == "step1"){
					...
					if($("#guest_name").val() == "" ){
						validation_error = 1;
						validation_message = validation_message + "<br/> Please enter guest name.";
					}						
					if($("#phone").val() == "" ){
						validation_error = 1;
						validation_message = validation_message + "<br/> Please enter contact number.";
					}						
					if($("#address").val() == "" ){
						validation_error = 1;
						validation_message = validation_message + "<br/> Please enter address.";
					}						
					...
					if(validation_error == 1 ){
						...
						//alert(validation_error+" msg = "+validation_message);
						validation_error = 0;
						validation_message = "";	
